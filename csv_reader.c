...
int main(void)
{
    ...
    // char *cell = index_2darray_csv(main_array, row_num, index);

    // printf("CELL:\n%s\n", cell);

    // // Split 2D array by comma into array of double link lists
    ...
    // char *cell = csv_reader_index(file, row, col, true);
    ...
    // // V4 - CSV DICT READER INDEX - Row is integer, Column is string (i.e. index into csv like a dict)
    ...
    //     if (cell != NULL) break;
    //     else printf("There is no column titled '%s' in the header of '%s'. Try again.\n", col, filename);
...
char *csv_reader_index(FILE *file, uintmax_t row, uintmax_t col, bool skip_header)
{
    ...
    {
        while ((tmp = fgetc(file)) != ',') if (tmp == EOF) {reached_eof = true; break;}
